...
const pNameLeft = document.getElementById('preloader-luke');
const pNameRight = document.getElementById('preloader-baffait');
...
const nameLeftChars = splitIntoChars(pNameLeft);
const nameRightChars = splitIntoChars(pNameRight);
const allChars = [...nameLeftChars, ...nameRightChars];
const allRevealEls = [...logoChar, ...nameLeftChars, ...nameRightChars];


function getCharGap() {
  return parseFloat(getComputedStyle(pNameRight).fontSize) * 0.08;
}

function layoutNames() {
  const fs = parseFloat(getComputedStyle(pNameRight).fontSize);
  if (!fs) return;
  const baselineOffset = -0.06; 

  const nameLeftOffset = pNameLeft.offsetLeft;
  const nameLeftWidth = pNameLeft.offsetWidth;
  const gapPx = fs * 0.08;

  const nameRightLeftPx = nameLeftOffset + nameLeftWidth + gapPx;
  pNameRight.style.left = (nameRightLeftPx / fs) + 'em';
  pNameRight.style.top = baselineOffset + 'em';

  const dotLeftPx = nameRightLeftPx + pNameRight.offsetWidth;
...
gsap.set(pNameLeft, { opacity: 1 });
gsap.set(pNameRight, { opacity: 1 });
...
function getTotalWidth() {
  return pLogo.offsetWidth + pNameLeft.offsetWidth + getCharGap() + pNameRight.offsetWidth + pDot.offsetWidth;
}
...
master
  ...
  .add(() => {
    ...
    const applyFinalState = () => {
      ...
      [pLogo, pNameLeft, pNameRight, pDot].forEach(el => {
...
async function setupScrollReveal() {
  ...
  [pContent, pLogo, pNameLeft, pNameRight, pDot].forEach(el => gsap.killTweensOf(el));
  ...
  scrollTl.fromTo(pNameLeft, { x: '0vw', opacity: 1 }, { x: exitLeft, opacity: 0, duration: 0.7, ease: 'none' }, 0.3);
  scrollTl.fromTo(pNameRight, { x: '0vw', opacity: 1 }, { x: exitRight, opacity: 0, duration: 0.7, ease: 'none' }, 0.3);
...
function setupProjectsSection() {
  ...
  ; (function () {
    ...
    (function () {
      ...
      var nameLeftEl = document.querySelector('.footer-name-luke');
      var nameRightEl = document.querySelector('.footer-name-baffait');
      var dotEl = document.querySelector('.footer-name-dot');
      if (!nameLeftEl || !nameRightEl) return;

      var nameLeftChars = rebuildChars(nameLeftEl, true);
      var nameRightChars = rebuildChars(nameRightEl, false);
      var dotChars = dotEl ? rebuildChars(dotEl, false) : [];

      
      var ordered = [];
      var nameLeftRev = nameLeftChars.slice().reverse();
      var rightSide = nameRightChars.concat(dotChars);
      var maxLen = Math.max(nameLeftRev.length, rightSide.length);
      for (var i = 0; i < maxLen; i++) {
        if (rightSide[i]) ordered.push(rightSide[i]);
        if (nameLeftRev[i]) ordered.push(nameLeftRev[i]);
      }
